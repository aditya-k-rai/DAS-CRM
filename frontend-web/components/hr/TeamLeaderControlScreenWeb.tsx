  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6 space-y-6 text-white rounded-3xl border border-slate-800">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <span className="text-xs font-black px-3 py-1 rounded-full border bg-amber-500/20 text-amber-300 border-amber-500/40">
          TEAM LEADER CONTROL SCREEN
        </span>
      </div>
      ...
      <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-400">👥 Employees Assigned Under {employee.name}</h3>
      <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 space-y-3">
        {employee.subordinates.map(sub => (
          <div key={sub.id} className="flex justify-between items-center text-xs border-b border-slate-800 pb-2 last:border-0">
            ...
            <span className="text-indigo-400 font-bold">📞 {sub.calls} Calls • 🎯 {sub.leads} Leads</span>
          </div>
        ))}
      </div>

      {/* Lead Status & Distribution Audit */}
      <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-400">📊 Unit Lead Status &amp; Distribution Audit</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <button onClick={() => { setLeadCategory('DISTRIBUTED'); setShowLeadPreviewModal(true); }} className="p-5 rounded-2xl bg-slate-900 border border-slate-800 text-left hover:border-slate-700">
          <p className="text-3xl font-black text-white">{employee.leads.totalReceived}</p>
          <p className="text-xs font-bold text-slate-400 mt-1">Got &amp; Distributed →</p>
        </button>
        ...
        <button onClick={() => { setLeadCategory('CONNECTED'); setShowLeadPreviewModal(true); }} className="p-5 rounded-2xl bg-slate-900 border border-slate-800 text-left hover:border-slate-700">
          ...
          <p className="text-xs font-bold text-slate-400 mt-1">Connected →</p>
        </button>
      ...
      <h3 className="text-sm font-extrabold uppercase tracking-wider text-slate-400">⚙️ Team Leader Controls</h3>
      ...
      {showLeadPreviewModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full space-y-3">
            <h4 className="text-sm font-extrabold text-white">🎯 Lead Distribution Audit — {leadCategory}</h4>
            {employee.leads.distributionBreakdown.map((item, idx) => (
              <div key={idx} className="p-3 bg-slate-950 border border-slate-800 rounded-xl flex justify-between items-center text-xs">
                ...
                <span className="text-amber-400 font-bold">{item.count} Leads</span>
              </div>
            ))}
      ...
      {showShareRolesModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full space-y-3">
            <h4 className="text-sm font-extrabold text-white">📜 Roles &amp; Responsibilities Report</h4>
            <p className="text-xs text-slate-300">TL Governance &amp; Rep Distribution SLA Sheet for {employee.name}</p>
            <button
              onClick={() => {
                alert(`📜 Shared Roles & Responsibilities Report for ${employee.name}!`);
