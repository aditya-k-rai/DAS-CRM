...
const DEALS: Record<string, any[]> = {
  prospecting: [
    { id: '1', title: 'Enterprise CRM Setup', company: 'TechCorp Ltd', value: '₹5,20,000', owner: 'JD', close: 'Aug 30', score: 72 },
    { id: '2', title: 'Digital Marketing Suite', company: 'AdAgency Pro', value: '₹1,80,000', owner: 'AS', close: 'Sep 15', score: 45 },
  ],
  qualification: [
    { id: '3', title: 'Real Estate Portal', company: 'Sunita RE', value: '₹8,50,000', owner: 'AS', close: 'Sep 5', score: 68 },
    { id: '4', title: 'Showroom Management', company: 'Lakshmi Auto', value: '₹2,40,000', owner: 'MK', close: 'Aug 25', score: 80 },
  ],
  proposal: [
    { id: '5', title: 'Interior Design Platform', company: 'Construkt Inc', value: '₹3,60,000', owner: 'JD', close: 'Aug 20', score: 85 },
  ],
  negotiation: [
    { id: '6', title: 'Hotel Management System', company: 'Grand Palace', value: '₹12,00,000', owner: 'JD', close: 'Aug 18', score: 91 },
  ],
  closed_won: [
    { id: '7', title: 'Auto Finance Tracker', company: 'SpeedCars', value: '₹4,50,000', owner: 'MK', close: 'Aug 10', score: 100 },
  ],
};
...
export function DealsKanban() {
  ...
  const stageTotal = (stageId: string) =>
    (deals[stageId] ?? []).reduce((s, d) => s + parseInt(d.value.replace(/[₹,]/g, '')), 0);
      ...
      <div className="crm-card mb-4 p-4">
        <div className="grid grid-cols-5 gap-4">
          {STAGES.map((stage) => {
            ...
            const count = (deals[stage.id] ?? []).length;
            return (
        ...
        {STAGES.map((stage) => {
          ...
          return (
            <div key={stage.id} className="kanban-column flex-shrink-0">
                  ...
                  <div key={deal.id} className="kanban-card">
                    ...
                    <div className="mb-3">
                      <div className="flex justify-between text-xs mb-1">
                        ...
                        <span className="font-medium" style={{ color: deal.score > 80 ? 'rgb(34,197,94)' : deal.score > 60 ? 'rgb(245,158,11)' : 'rgb(239,68,68)' }}>
                          {deal.score}%
                        </span>
                      </div>
                      <div className="h-1.5 rounded-full overflow-hidden" style={{ background: 'rgb(var(--border))' }}>
                        <div className="h-full rounded-full transition-all" style={{
                          width: `${deal.score}%`,
                          background: deal.score > 80 ? '#22c55e' : deal.score > 60 ? '#f59e0b' : '#ef4444',
                        }} />
                      </div>
                    </div>

                    <div className="flex items-center justify-between">
                      <span className="text-sm font-bold" style={{ color: 'rgb(var(--brand-400))' }}>{deal.value}</span>
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1 text-xs text-muted">
                          <Calendar size={11} />
                          {deal.close}
                        </div>
                        <div className="avatar w-6 h-6 text-xs" style={{ fontSize: '10px' }}>{deal.owner}</div>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {/* Add deal */}
              <button className="w-full mt-2 py-2 rounded-lg text-xs text-muted border border-dashed transition-all hover:border-brand-500 hover:text-white flex items-center justify-center gap-1" style={{ borderColor: 'rgb(var(--border))' }}>
