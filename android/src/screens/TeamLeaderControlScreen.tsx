 ...
 * Includes: Name, Email, Phone, Role Upgrade, Change Supervisor, Employees Assigned Under management (change/add rep with lead data),
...
export default function TeamLeaderControlScreen({ employee, onBack, onUpdateEmployee }: Props) {
  ...
  const [leadCategory, setLeadCategory] = useState<'DISTRIBUTED' | 'CONNECTED' | 'NEGOTIATED' | 'MEETING_DONE' | 'WON'>('DISTRIBUTED');
  ...
  const [manageRepsModalOpen, setManageRepsModalOpen] = useState(false);
  ...
  return (
    <View style={[styles.container, { paddingTop: topPadding }]}>
      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingBottom: bottomPadding + 30 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.profileCard}>
          ...
          <View style={styles.supBox}>
            <Text style={styles.supLabel}>Assigned Under Manager:</Text>
        ...
        {/* Employees Assigned Under TL Section */}
        <View style={{ width: '100%', maxWidth: 600, marginBottom: 14 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <Text style={styles.sectionTitleNoMargin}>👥 Employees Assigned Under {employee.name}</Text>
            <TouchableOpacity style={styles.actionChipBtn} onPress={() => setManageRepsModalOpen(true)}>
              <Text style={styles.actionChipBtnText}>Add / Change Reps ✏️</Text>
            </TouchableOpacity>
          </View>
        ...
        {/* Lead Status & Distribution Audit Section */}
        <Text style={styles.sectionTitle}>📊 Unit Lead Status &amp; Distribution Audit</Text>
        <View style={styles.grid2}>
          <TouchableOpacity style={[styles.statCard, { borderColor: 'rgba(99,102,241,0.4)' }]} onPress={() => { setLeadCategory('DISTRIBUTED'); setLeadPreviewModalOpen(true); }}>
        ...
        <Text style={styles.sectionTitle}>⚙️ Team Leader Operations</Text>
        <View style={{ gap: 10, width: '100%', maxWidth: 600 }}>
          ...
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: 'rgba(99,102,241,0.2)', borderColor: '#818cf8' }]} onPress={() => setShareRolesModalOpen(true)}>
            <Text style={[styles.actionBtnText, { color: '#a5b4fc' }]}>📜 Share Roles &amp; Responsibilities Report →</Text>
          </TouchableOpacity>
        </View>
      ...
      <Modal visible={changeSupervisorModalOpen} transparent animationType="slide">
        <View style={styles.overlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>✏️ Change Assigned Manager</Text>
      ...
      <Modal visible={manageRepsModalOpen} transparent animationType="slide">
        <View style={styles.overlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>👥 Manage Subordinate Reps</Text>
            <Text style={{ color: '#94a3b8', fontSize: 11, marginBottom: 8 }}>Re-assign or add Sales Reps under {employee.name}:</Text>
            {employee.subordinates.map(sub => (
              <View key={sub.id} style={styles.leadItem}>
                <Text style={{ color: '#ffffff', fontWeight: '800' }}>{sub.name}</Text>
                <Text style={{ color: '#34d399', fontSize: 10 }}>{sub.leads} Leads</Text>
              </View>
            ))}
            <TouchableOpacity style={styles.submitBtn} onPress={() => setManageRepsModalOpen(false)}>
              <Text style={styles.submitBtnText}>Save Subordinates →</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={leadPreviewModalOpen} transparent animationType="slide">
        <View style={styles.overlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>🎯 Lead Audit — {leadCategory}</Text>
            {employee.leads.distributionBreakdown.map((item, idx) => (
              <View key={idx} style={styles.leadItem}>
                <View>
                  <Text style={{ color: '#ffffff', fontWeight: '800' }}>To: {item.targetName}</Text>
                  <Text style={{ color: '#94a3b8', fontSize: 10 }}>{item.dateStr}</Text>
                </View>
                <Text style={{ color: '#fbbf24', fontWeight: '800' }}>{item.count} Leads</Text>
              </View>
            ))}
      ...
      <Modal visible={shareRolesModalOpen} transparent animationType="slide">
        <View style={styles.overlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>📜 Roles &amp; Responsibilities</Text>
            <Text style={{ color: '#ffffff', fontSize: 11 }}>TL Governance &amp; Rep Distribution SLA Sheet for {employee.name}</Text>
            <TouchableOpacity
              style={styles.submitBtn}
              onPress={() => {
                const text = `📜 *TL Governance & Rep SLA Sheet*\n👤 *TL:* ${employee.name}\n🏢 *Supervisor:* ${employee.assignedManager}\n👥 *Assigned Reps:* ${employee.subordinates.length}`;
...
const styles = StyleSheet.create({
  ...
  roleTag: { backgroundColor: 'rgba(251,191,36,0.2)', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: '#fbbf24' },
  roleTagText: { color: '#fbbf24', fontSize: 9, fontWeight: '900' },
  ...
  avatar: { width: 50, height: 50, borderRadius: 25, borderWidth: 2, borderColor: '#fbbf24' },
