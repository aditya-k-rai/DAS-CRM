...
export default function MoreControlsScreen({
  ...
  // Dedicated Full Screen View Renderer for Active Button
  if (activeModal !== null) {
    const activeBtn = GRID_BUTTONS.find((b) => b.key === activeModal);
    return (
      <View style={styles.container}>
        {/* Dedicated Screen Top Navigation Header */}
        <View style={styles.modalTopBar}>
          <TouchableOpacity
            style={[styles.actionBtn, { backgroundColor: '#1e293b', borderWidth: 1, borderColor: '#334155', paddingHorizontal: 12, paddingVertical: 6 }]}
            onPress={() => setActiveModal(null)}
          >
            <Text style={{ color: '#38bdf8', fontWeight: '900', fontSize: 11 }}>← Back to Operations</Text>
          </TouchableOpacity>
          <Text style={styles.modalTopTitle}>{activeBtn?.icon} {activeBtn?.label}</Text>
        </View>
        ...
        <View style={{ flex: 1 }}>
          ...
          {/* Active Dedicated Screen View Content */}
          {activeModal === 'WA_TEMPLATES' && (
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
              <View style={styles.moduleCard}>
      ...
      <Modal visible={editTplModalOpen} transparent animationType="slide">
        ...
        {/* ── EDIT / CREATE WHATSAPP TEMPLATE SUB-MODAL ──────────────────────────── */}
        <Modal visible={editTplModalOpen} transparent animationType="slide">
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              <View style={styles.modalHeader}>
