...
export class MailService {
  ...
  async sendSuperAdminOtp(email: string, otp: string): Promise<void> {
    await this.transporter.sendMail({
      from: `"NexCRM Security" <${process.env.SMTP_FROM}>`,
      to: email,
  ...
  async sendRoleTransitionNotification(
    ...
    await this.transporter.sendMail({
      from: `"NexCRM" <${process.env.SMTP_FROM}>`,
      to: userEmail,
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px;">
          ...
          <a href="${process.env.FRONTEND_URL}/dashboard" style="display: inline-block; margin-top: 16px; padding: 12px 24px; background: #6366f1; color: white; border-radius: 8px; text-decoration: none; font-weight: bold;">
    ...
    await this.transporter.sendMail({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px;">
          ...
          <a href="${process.env.FRONTEND_URL}/admin/audit-logs" style="display: inline-block; margin-top: 16px; padding: 10px 20px; background: #f3f4f6; color: #111827; border-radius: 8px; text-decoration: none; font-weight: bold; border: 1px solid #d1d5db;">
            View Audit Logs →
          </a>
        </div>`,
    });
  ...
  async sendPlanUpgradeRequestNotification(
    ...
    await this.transporter.sendMail({
      from: `"NexCRM Billing" <${process.env.SMTP_FROM}>`,
      to: superAdminEmail,
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e5e7eb; border-radius: 12px;">
          ...
          <a href="${process.env.FRONTEND_URL}/admin/super" style="display: inline-block; margin-top: 16px; padding: 12px 24px; background: #6366f1; color: white; border-radius: 8px; text-decoration: none; font-weight: bold;">
            Review & Approve →
          </a>
        </div>`,
    });
