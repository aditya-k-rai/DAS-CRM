...
export class CustomFieldsService {
  ...
  async create(organizationId: string, dto: {
    ...
    const fieldTypeEnum = (dto.fieldType === 'TEXTAREA' ? 'TEXT' : dto.fieldType) as CustomFieldType;
    ...
    if (existing) throw new BadRequestException(`Field '${key}' already exists for ${entity}`);
  ...
  async update(organizationId: string, id: string, dto: Partial<{
    label: string; fieldLabel: string; isRequired: boolean; options: string[]; order: number;
  }>) {
  ...
  async validateCustomFields(organizationId: string, entityType: string, values: Record<string, any>) {
    ...
    for (const def of defs) {
      ...
      if (val !== undefined && val !== null && def.fieldType === 'DROPDOWN' && opts.length > 0) {
        if (!opts.includes(val)) {
          throw new BadRequestException(`Invalid option '${val}' for field '${def.label}'.`);
        }
      }
    }
    return true;
